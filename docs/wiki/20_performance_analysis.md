...
# 分析実行
results = analyze_fingerprint_generation(test_files, configs)
...
db_results = analyze_database_search_performance(
    mimizam_instances, 
    query_files,
...
# レポート生成
report = generate_database_performance_report(db_results)
print(report)
```
