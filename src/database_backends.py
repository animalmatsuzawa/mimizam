...
from .exceptions import ConfigurationError, log_and_raise
...
def create_database_backend(config: DatabaseConfig) -> DatabaseBackend:
    ...
    if backend_type not in backend_map:
        ...
        log_and_raise(logger, ConfigurationError, 
                     f"Unsupported backend: {config.backend}. Supported backends: {supported_backends}")
    
    # バックエンドクラスの利用可能性チェック
    if backend_class is None:
        log_and_raise(logger, ConfigurationError, 
                     f"{config.backend} backend implementation is not available. Please check that required libraries are installed.")
